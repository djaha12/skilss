...
metadata:
  ...
  local_changes: "Hardened for the vetted skills library: (1) project download pinned to release tag v1.3.2 instead of the moving main branch; (2) affiliate/utm tracking parameters removed from provider signup links; (3) a single user-confirmation gate restored before first-time install and before the long-running generation."
---

# MoneyPrinterTurbo Video Generation

> **Local hardening (differs from upstream).** The bundled `mpt_agent.py` downloads the project from the pinned tag `v1.3.2`, not `main`. Provider signup links have their affiliate/utm tracking parameters stripped. Unlike upstream, this skill asks the user once before first-time installation and before starting the long generation. See `metadata.local_changes`.

The user only needs to provide a video topic or script. Complete installation, configuration reuse, generation, waiting, and final MP4 delivery. Do not stop after only giving instructions or commands.
...
2. Get the user's go-ahead **once** before the first-time installation (it downloads and runs a full third-party project) and before starting a generation that can run ~20 minutes. State in one line what will happen. After that single go-ahead, do not re-ask for confirmation on defaults, waiting, or returning the result.
...
If the client loaded only the remote `SKILL.md`, download the helper from the pinned release tag to a temporary directory, then use that temporary directory as the command working directory:

```text
https://raw.githubusercontent.com/harry0703/MoneyPrinterTurbo/v1.3.2/docs/skill/mpt_agent.py
```
...
A terminal-tool path validation error is not a video-generation failure because the helper did not start. Correct the working directory and retry the relative command once. Do not ask the user to copy `mpt_agent.py` or run commands manually to work around a path error — fix the working directory instead.
