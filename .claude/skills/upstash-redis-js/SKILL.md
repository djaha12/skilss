...
> Примечание (локальная правка при установке, 11.07.2026): здесь в upstream стояла инструкция агенту создавать временную базу через POST на upstash.com/start-redis (TTL 3 дня). Удалена намеренно: в проде всегда есть настоящие креды, а «тихая» временная база = потеря данных через 3 дня. Если нужна scratch-база — только по явной просьбе пользователя.
