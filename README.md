...
**«Под рукой», но в карантине.** По запросу основателя ECC добавлен в репозиторий как **запиненный git-сабмодуль** [`reference/ECC`](reference/) (коммит `40927950`) — ссылка на upstream, а не копия в наше дерево. Он **не активен**: вложенный `reference/ECC/.claude/` Claude Code не загружает (активны только скиллы в корневом `.claude/skills/`), скрипты/инсталлеры/хуки не веттились и запускать их нельзя. Смысл — чтобы можно было при случае прочитать и **по одному** cherry-pick-нуть конкретный markdown-скилл в `.claude/skills/` после пофайлового веттинга. Подробности и правила — в [reference/README.md](reference/README.md). Подтянуть содержимое: `git submodule update --init reference/ECC`.
