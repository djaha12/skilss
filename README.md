...
| [iart-ai/webgl-animation-skills](https://github.com/iart-ai/webgl-animation-skills) — iart.ai | MIT | 3D/WebGL: 3 скилла |
...
| Скилл | Что делает |
|---|---|
| `threejs-animation` | Three.js-сцены: render loop на `clock.getDelta()`, движение камеры и привязка к скроллу, GLTF-клипы через `AnimationMixer` с кроссфейдом, `InstancedMesh`, R3F, постобработка. Отдельно — полная дисциплина `dispose()` для GPU-ресурсов и harness `?t=N` для детерминированных скриншотов |
| `shader-glsl` | Фрагментные шейдеры: шум/fbm, domain warping, SDF, градиенты и плазма, туман и свечение, переходы и дисторшн, `ShaderMaterial` с униформами |
| `particle-system` | Частицы: снег, дым, пыль, искры, confetti; flow field и curl noise, connected-dot сети, GPU `Points` на десятки тысяч частиц |

В каждом скилле лежит `scripts/` — `seek-shot.sh` (заморозить анимацию на `?t=N` и снять кадр через Playwright), `contact-sheet.sh` (склеить start/mid/end в один лист через ffmpeg), `probe-mp4.sh` (проверить реальные параметры mp4 через ffprobe). *Локальная правка: в трёх reference-файлах промо-футер апстрима с UTM-ссылками на iart.ai заменён на обычную строку атрибуции — трекинг в контенте скилла, который читает агент, ни к чему.*
...
Раунд 3D/WebGL (17.08.2026), тот же чеклист: **3/3 чисто, 0 опасных**. `iart-ai/webgl-animation-skills` — скрытых символов, HTML-комментариев, prompt-injection и эксфильтрации нет; из внешних ссылок в скиллах были только промо-футеры на iart.ai (сняты, см. выше); три shell-скрипта (`seek-shot`/`contact-sheet`/`probe-mp4`) написаны аккуратно — `set -euo pipefail`, переменные в кавычках, вызовы только локальных ffmpeg/ffprobe/playwright, никаких `curl … | sh` и eval. **Оговорка по репутации:** у репозитория ~7★ и он молодой (июнь 2026), то есть ниже планки, по которой мы отклоняли community-скиллы. Установлен по прямому решению основателя; страховка — вендоринг вместо `/plugin install`, так что контент запинен и апстрим-правки сюда не доедут без пересинхронизации.
