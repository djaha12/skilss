...
### Кастомные скиллы GARAJ.KG (свои, не из экосистемы)

Написаны под проект `auto-superapp-kg` и дистиллированы из его реальной боли — это самая ценная часть
библиотеки, потому что такого знания в открытой экосистеме нет. Заведены здесь 24.07.2026: до этого они
существовали в единственном экземпляре в рабочем репозитории, без резервной копии.

| Скилл | Что делает |
|---|---|
| `garaj-known-traps` | Каталог капканов и вечных правил кодовой базы, которые уже жгли прод или основателя |
| `garaj-session-protocol` | Протокол сессии: русские коммиты, ветки `claude/*`, журнал в CLAUDE.md, гейт перед мержем |
| `actions-as-hands` | Карта воркфлоу GitHub Actions — единственные руки агента на проде (в песочнице нет сети) |
| `kv-data-patterns` | Модель данных Upstash Redis через свой `kv.ts`: фенсинг-локи, fail-closed деньги, шардинг, сухие миграции |
| `app-store-lifecycle` | Жизненный цикл в сторах: 3 реальных прецедента реджекта Apple и анти-стиринг-гардрейл |
| `submit-appstore` | Отправка обновления iOS в App Store без ручных кликов в App Store Connect |
| `garaj-i18n-parity` | Обязательная процедура добавления строки интерфейса: 4 словаря + счётчик паритета, иначе CI красный |
| `founder-runbook-writer` | Написание ранбуков для основателя (`*-SETUP.md` / `*-CHECKLIST.md`) в жанре репозитория |
| `garaj-content-week` | Недельный контент-план (7 дней, все площадки, только черновики — без автопостинга) |

⚠️ **Сверить перед следующим использованием.** Ресёрч 24.07.2026 нашёл в них протухшие факты:
в `app-store-lifecycle` и `garaj-known-traps` (правило №15) ключевой гейт назван `useIsNative`, а в коде
с 20.07 это `useHidePaid` (`native-bridge.ts:234`); из 7 проверенных якорей строк 4 указывают не туда;
в `actions-as-hands` описан ритуал с `vercel_token`-инпутом, которого больше нет — 22.07 креды переехали
в GitHub Environment `production-secrets`. Подробности — в [SKILLS-RESEARCH-2026-07-24.md](SKILLS-RESEARCH-2026-07-24.md).
...
**Известный дрейф upstream на 24.07.2026** (проверено, но ещё не применено): obra/superpowers вышел v6.2.0 — изменились все 10 установленных скиллов; в anthropics/skills переписаны `claude-api` (PR #1463) и `docx`/`pptx`/`xlsx` (PR #1447, часть файлов удалена — наши копии на них ссылаются); в marketingskills `ad-creative` 2.6.0 → 2.8.0 и `video` 2.0.1 → 2.1.0.

## Синхронизация с рабочим репозиторием

Библиотека — склад; работает агент со своей копией в `auto-superapp-kg/.claude/skills`. На 24.07.2026 копии разошлись:
здесь 91 скилл, там 56, общих 47. Кастомные GARAJ-скиллы перенесены сюда этим раундом, **обратное направление
ещё не сделано** — 35 скиллов библиотеки (весь цикл `grill-me`/`to-tickets`/`implement`/`handoff`, `research`,
`skill-creator`, `mcp-builder`, `analytics`, `launch`, `marketing-plan`, `customer-research`, офисные docx/pdf/pptx/xlsx
и др.) в рабочем репозитории отсутствуют, то есть в реальной работе над проектом недоступны. Переносить нужно
из сессии с правом записи в `auto-superapp-kg`.
