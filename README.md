...
Отобранная библиотека агентских скиллов для Claude Code. Источники (ядро — по состоянию на 11 июля 2026; более поздние добавления помечены датой):
...
| [dair-ai/dair-academy-plugins](https://github.com/dair-ai/dair-academy-plugins) — DAIR.AI (авторы курсов dair.ai), _добавлено 21.07.2026_ | open source, отдельного LICENSE-файла нет | AI-разведка из X: 1 скилл |
...
### Разведка и мониторинг (DAIR.AI)

| Скилл | Что делает |
|---|---|
| `x-agent-intelligence` | Собирает читаемую локальную ленту новостей об ИИ и агентах из официального X (Twitter) MCP-сервера: резолвит хендлы источников, нормализует посты, ранжирует (релевантность/качество источника/свежесть, не только engagement) и рендерит самодостаточный `feed.html` — поиск, фильтры по категориям, группировка по дням, аватары, ссылки на оригинал, панель источников с local-storage. Только чтение X MCP, без бэкенда и планировщика: расписание (напр. каждые 4 ч) вешаете сами. Требует настроенного X MCP — гайд в `references/x-mcp-setup.md`, стартовый список аккаунтов в `references/starter-sources.md`, эталонный дизайн ленты в `assets/reference-artifact.html`. *Локальная правка: пути `../../references/…` и `../../assets/…` в SKILL.md заменены на скилл-относительные под плоскую раскладку `.claude/skills/`.* |
...
Каждый сторонний скилл перед установкой прошёл проверку: чтение всех файлов, поиск prompt-injection, эксфильтрации, скрытых инструкций (zero-width символы, HTML-комментарии), внешних эндпоинтов, install-хуков и деструктивных команд + адверсариальная верификация флагов. Результат 11–12.07.2026 (три раунда): **57/57 чисто, 0 опасных**. Закрытые поверхности: `web-design-guidelines` тянул правила с изменяемого URL — запинен локально; `upstash-redis-js` содержал инструкцию агенту создавать временную базу (TTL 3 дня) — удалена. Подробности и чеклист — в [SKILLS-RESEARCH-2026-07-11.md](SKILLS-RESEARCH-2026-07-11.md). **Добавление 21.07.2026:** `x-agent-intelligence` (DAIR.AI) проверен по той же методике — самодостаточный HTML-артефакт (инлайн `data:`-картинки, внешние ссылки только на `x.com`, без внешних скриптов, трекеров, `fetch`/XHR и секретов; `esc()` через `textContent` защищает от XSS), в setup-гайде только плейсхолдерные токены; чисто. Итог 58/58.
