...
Отобранная библиотека агентских скиллов для Claude Code. Источники (по состоянию на 11 июля 2026; MoneyPrinterTurbo, freecut и scroll-world — 16 июля):
...
| [harry0703/MoneyPrinterTurbo](https://github.com/harry0703/MoneyPrinterTurbo) — генератор AI-видео | MIT | 1 скилл (`moneyprinterturbo-video`), укреплён локально |
| [walterlow/freecut](https://github.com/walterlow/freecut) — веб-видеоредактор | MIT | 2 скилла (`changelog`, `translate-app-locales`), обобщены под любой проект |
| [oso95/scroll-world](https://github.com/oso95/scroll-world) — плагин: лендинги-«полёт сквозь мир» через Higgsfield | MIT | 1 скилл (`scroll-world`), добавлен как есть |
...
### Медиа/креатив и dev-утилиты

Взяты из репозиториев-приложений и плагинов (не из обычных скилл-библиотек). MPT укреплён, два скилла freecut обобщены и очищены от привязки к их проекту; `scroll-world` добавлен как есть — проверка чистая, править нечего.

| Скилл | Что делает |
|---|---|
| `moneyprinterturbo-video` | Генерация готового видео из темы/сценария через MoneyPrinterTurbo (self-install, uv, CLI). **Укреплён**: загрузка проекта запинена на релиз-тег `v1.3.2` (вместо ветки `main`), убраны affiliate/utm-параметры из ссылок провайдеров, возвращён один гейт подтверждения перед установкой и длинным прогоном. ⚠️ Тянет тяжёлый Python-проект + сеть + ~20 мин на прогон — в песочницах без сети/на слабом канале работать не будет, лучше локально |
| `changelog` | Курирование git-коммитов в user-facing чейнджлог (Keep a Changelog): режимы draft/append/rollup/backfill, «user-facing test», verify-net-state (added-then-removed → 0), launch-vs-increment, правила отсева шума и переписывания dev-speak. **Обобщён** с freecut: убраны пути/схема/инциденты конкретного проекта, схема версий определяется по существующему `CHANGELOG.md` |
| `translate-app-locales` | Добавление/аудит i18n-переводов: покрытие всех ключей по base + partial неймспейсам, сохранение плейсхолдеров/плюралей/тегов, bundled read-only скрипт `check-locale-coverage.mjs`. **Обобщён** с freecut: конвертирован из Codex-скилла в Claude Code, удалены «FreeCut Locale Map» и захардкоженный путь автора |
| `scroll-world` | Иммерсивный scroll-scrubbed лендинг («полёт сквозь мир»): интервью по бренду/сценам → генерация кадров + бесшовных камер-клипов через **Higgsfield** (Seedance/Kling image→video) → портируемый vanilla-JS scrub-движок (blob-seek, ленивая загрузка, seam-кроссфейд, mobile-хардненинг). Добавлен как есть (проверка чистая). ⚠️ Нужен внешний Higgsfield CLI (auth + кредиты) + `ffmpeg`, опц. Codex CLI — в песочнице без сети не отработает, запускать локально |
...
**Раунд 16.07.2026 — MoneyPrinterTurbo + freecut** (адверсариальный workflow, 3 кандидата × независимая проверка + попытка опровержения). Скрытого юникода, HTML-комментариев и эксфильтрации не найдено ни в одном. `moneyprinterturbo-video` не вредонос (есть защита от zip-slip, ключи не печатаются), но качал непинованный `main.zip`, нёс affiliate/utm-ссылки и инструкцию обходить подтверждение — всё **укреплено** перед добавлением (пиннинг на `v1.3.2`, чистые ссылки, гейт подтверждения). Скиллы freecut (`changelog`, `translate-app-locales`) безопасны, но были привязаны к их проекту (а `translate` — вообще Codex-скилл) — **обобщены** и переведены в формат Claude Code. Bundled `check-locale-coverage.mjs` — строго read-only, без сети/exec, оставлен как есть.

**Раунд 16.07.2026 — scroll-world** (независимый адверсариальный security-ревьюер, попытка опровергнуть «чисто»). Вердикт **CLEAN**: без эксфильтрации, без XSS для посетителя (весь конфиг экранируется `esc()`), без prompt-injection, без деструктивных команд (`rm` нигде нет), без скачивания удалённого кода. `fetch`/`curl` только *качают* ассеты — локальные из конфига и сгенерированные по `result_url` из ответа авторизованного Higgsfield API. `knockout.py` — чистый PIL (flood-fill), без сети/subprocess. `codex exec` вызывается в песочнице (`-s workspace-write`, cwd в скретче). Два defense-in-depth замечания задокументированы: (1) схема `href` в конфиге движка не валидируется — `javascript:`-URI сработал бы по клику, но достижимо только через собственный конфиг автора страницы (self-XSS), не через посетителя; (2) `esc()` не экранирует одинарную кавычку — корректно для текущих sink'ов (двойные кавычки/текст), важно учесть при переносе движка во фреймворк. Higgsfield/ffmpeg/Codex — внешние CLI, которые скилл *использует*, а не бандлит; скилл добавлен как есть, без правок.
