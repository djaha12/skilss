...
## Дополнение — раунд 4 (13.07.2026): ECC / Everything Claude Code

По присланному промо изучён [`affaan-m/ECC`](https://github.com/affaan-m/ECC) («Everything Claude Code»; ранее так и назывался, канонический слаг маркетплейса/плагина — `ecc@ecc`). **Вывод: не вендорится в библиотеку; ставить отдельно через `/plugin` тем, кому нужно.** В README добавлены разбор-раздел и пойнтер на установку.

**Что это.** Не набор markdown-скиллов, а харнесс-система (MIT OSS + платный ECC Pro как GitHub App), мультихарнесс (Claude Code, Cursor, Codex, Gemini, Zed, Copilot…):

- **278 скиллов, 67 агентов, 94 legacy-command-шима, 34 правила**, 6 встроенных MCP-серверов (GitHub, Context7, Exa, Memory, Playwright, Sequential Thinking);
- **авто-хуки** на `PreToolUse`/`PostToolUse`/`SessionStart`/`PreCompact`/`Stop`/`SessionEnd` — перехватывают shell, правки файлов, MCP (блокировщик dev-сервера, pre-commit-проверки, prettier/TS-гейты, PR-логгер и т.д.);
- **self-learning** (Continuous Learning v2): `evaluate-session.js` извлекает паттерны из сессий, «инстинкты» пишутся в `CLV2_HOMUNCULUS_DIR` (по умолчанию `~/.local/share/ecc-homunculus`), memory-хуки — под `~/.claude`;
- исполняемая поверхность: `install.sh`, `install.ps1`, кросс-платформенные Node-скрипты, Python/Tkinter-дашборд (`ecc_dashboard.py`); npm `ecc-universal` и `ecc-agentshield` (AgentShield: заявлено 102 правила / 1282 теста, секрет-детект, аудит permission/hook/MCP).

**Почему отклонено к вендорингу (совпадает с уже действующими правилами репозитория):**

1. Это **плагин с серверным/скриптовым кодом и авто-хуками**, а не отдельные скиллы — вендорить как markdown нельзя; аналогично уже принятому решению по telegram/playwright ставить такое целиком через `/plugin`.
2. **Коллизия описаний:** 278 скиллов + 67 агентов поверх наших 91 — тот самый «вред двойного срабатывания», из-за которого ранее отклонены дубли superpowers; здесь эффект ×3.
3. **Скрипт/хук-поверхность не проходила наш пофайловый веттинг** (авто-хуки на shell/правки/MCP + self-learning, читающий сессии — приоритетный вектор по OWASP AST01; сертифицировать по README нельзя). По README явной эксфильтрации/телеметрии не заявлено, но это не заменяет построчного ревью (вывод ToB: README-уровень не гарантирует ничего).
4. **Заточено под автора:** предупреждение самого промо — «чужая система… скопировать 1:1 и ждать магии не выйдет»; и «не ставить всё сразу — иначе путаница из 67 агентов».
5. **Профиль:** покрытие (Go/Java/Kotlin/C++/F#/Rust/Django/Spring/Laravel/Quarkus/Perl…) в основном вне профиля GARAJ.KG и его анти-потребностей.

**Что полезно взять отдельно, не принимая систему:** **AgentShield** как самостоятельный пре-инсталл/аудит-сканер — `npx ecc-agentshield scan` (`--opus` — состязательная кросс-проверка тремя агентами). Ложится рядом с уже рекомендованным NVIDIA SkillSpector в пайплайн проверки будущих установок (помня вывод ToB: любой сканер обходим, не заменяет ревью).

**Установка (для желающих), в своей сессии Claude Code:**
...
Caveats при установке: выбирать минимальный набор; перед включением авто-хуков/self-learning прогнать полный веттинг; в MCP-конфигах заменить `YOUR_*_HERE` на свои ключи.

*Заметка по терминологии:* строка «everything-claude-code — сборник ссылок» в README относилась к awesome-агрегаторам; ECC (он же everything-claude-code у affaan-m) — не список ссылок, а именно эта плагин-система. README уточнён.

**Адверсариальная перепроверка «нужен ли он нам» (13.07.2026).** Прогнан панель-разбор из 4 независимых линз (адвокат внедрения, скептик, security, overlap) + синтез-судья. Секьюрити-ленс реально прочитал исходники хуков ECC: они **локальные, редактят секреты (`ghp_`/`AKIA`/Authorization), наружу ничего не шлют** — опасность ниже, чем ждали; но структурная претензия (ambient-исполнение без согласия на каждый tool-use + самопишущийся стор «инстинктов», плюс `auto-update.js` пробрасывает весь `process.env`) остаётся. Консенсус (3 из 4 «не нужно», 1 «nice-to-have»): **ECC целиком, 67 агентов и хуки/self-learning — нет** (профиль, коллизии, неветтируемая исполняемая поверхность); **отдельные скиллы — case-by-case** (конкретного пробела не нашли); **AgentShield — опционально** (полезен, но перекрывается SkillSpector + ToB `agentic-actions-auditor`, и это сканер того же автора — trust-circular). Единственная реальная дыра, которую вскрыл разбор, — **у нас, а не в ECC**: все скиллы совещательные, детерминированного гейта качества нет; чинится через уже рекомендованный `hookify` + pre-commit/PostToolUse-хук поверх Vitest, **без** ECC.

**Действие по запросу основателя («ладно добавь, может пригодится»).** ECC добавлен как **запиненный git-сабмодуль** `reference/ECC` (коммит `40927950`) — в карантин, НЕ в `.claude/skills/`. Не активируется (вложенный `.claude/` не загружается), скрипты не запускать; предназначен для чтения и поштучного cherry-pick после веттинга. Правила — в `reference/README.md`. Это ссылка на upstream, а не вендоринг: принцип «не копируем серверный/скриптовый код в дерево» сохранён.
